...
// first flagged as a recurring pattern in docs/CLAUDE_B_SESSION_
// MIGRATION_20260919B.md §4): module-level collector for the pos-as-array
// import-pipeline fix below — every OCR-batch `pos` array coercion (or
// unresolved non-single-element array) gets logged here instead of
// silently vanishing to null. Written to docs/POS_ARRAY_COERCIONS.md at
// the end of main(), same pattern as pickPrimaryVerifiedTies above.
const posArrayCoercions = [];
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      parsed.forEach(item => {
        ...
        // BUGFIX (2026-09-20, Claude B, docs/CLAUDE_B_SESSION_MIGRATION_
        // 20260920.md §6.3, recurring pattern first flagged docs/
        // CLAUDE_B_SESSION_MIGRATION_20260919B.md §4 — "pos-as-array is
        // now a confirmed recurring bug pattern in OCR-batch handoffs
        // (seen in both a prior session's pending-lexicon workflow pairs
        // and [that same] session's P24/P25 batch) — worth a real prepare-data.js/
        // import-pipeline fix (coerce or reject array pos explicitly,
        // rather than silently nulling it) if it surfaces a third time".
        // It surfaced a third time (this session's own inspection of docs/
        // CLAUDE_D_20260918_p24_p25_ready_for_a.json: 84 rows, every one
        // an array of exactly one string, e.g. `["v."]` — Claude D's OCR-
        // batch handoff format wraps pos in an array even though it's
        // always exactly one tag; those 84 rows were only clean in
        // master_dictionary.json today because Claude A manually unwrapped
        // each by hand during promotion, commit 28b990e — the underlying
        // pipeline gap was never actually fixed). The old `typeof
        // item.pos === 'string'` check silently discarded ANY array shape
        // to null with zero trace, which is exactly what let this recur
        // unnoticed. Now: a single-element string array is coerced (safe —
        // matches every real instance found, no ambiguity) and logged to
        // docs/POS_ARRAY_COERCIONS.md so it's visible instead of silent.
        // Anything else (empty array, 2+ elements, non-string element) is
        // explicitly NOT guessed at — a genuine multi-sense/ambiguous pos
        // is a content question for Claude A, not an engineering one — but
        // it's now visibly reported as unresolved instead of silently
        // vanishing, which is the actual bug this fix closes: not "arrays
        // are unsupported" but "arrays were unsupported AND invisible".
        let pos;
        if (typeof item.pos === 'string') {
          pos = item.pos.trim() || null;
        } else if (Array.isArray(item.pos)) {
          if (item.pos.length === 1 && typeof item.pos[0] === 'string' && item.pos[0].trim()) {
            pos = item.pos[0].trim();
            if (eng) posArrayCoercions.push({ key: eng, result: pos, shape: 'single-element array, coerced' });
          } else {
            pos = null;
            if (eng) posArrayCoercions.push({ key: eng, result: null, shape: `array(length ${item.pos.length}), NOT coerced — ambiguous` });
          }
        } else {
          pos = null;
        }
...
function main() {
  ...
  if (posArrayCoercions.length) {
    const coercedCount = posArrayCoercions.filter(c => c.result !== null).length;
    const unresolvedCount = posArrayCoercions.length - coercedCount;
    console.log(`pos-as-array report: ${posArrayCoercions.length} row(s) (${coercedCount} coerced, ${unresolvedCount} unresolved), see docs/POS_ARRAY_COERCIONS.md`);
    const posLines = [
      '# pos-as-array coercion report',
      '',
      'Auto-generated by prepare-data.js. Regenerated on every build; do not',
      `hand-edit. ${posArrayCoercions.length} row(s) currently listed (${coercedCount}`,
      `coerced, ${unresolvedCount} unresolved).`,
      '',
      'Engineering fix (2026-09-20, Claude B, docs/CLAUDE_B_SESSION_',
      'MIGRATION_20260920.md §6.3), closing a recurring OCR-batch-handoff',
      'pattern first flagged in docs/CLAUDE_B_SESSION_MIGRATION_20260919B.md',
      '§4: some import sources ship `pos` as an array (e.g. `["v."]`)',
      'instead of a bare string. Previously silently discarded to `null`',
      'with zero trace. Now: a single-element string array is coerced to',
      'its string value (every real instance found so far is this shape,',
      'so no ambiguity) and logged below. A row marked "NOT coerced" has a',
      'genuinely ambiguous shape (empty array, or 2+ elements — a real',
      'multi-sense pos) and still resolves to `null`, same as before, but',
      'is now visible here instead of vanishing — that ambiguity is a',
      'content question for Claude A, not an engineering one to guess at.',
      '',
      ...posArrayCoercions
        .slice()
        .sort((a, b) => a.key.localeCompare(b.key))
        .map(({ key, result, shape }) => `- \`${key}\`: ${shape}${result !== null ? ` — coerced to \`${result}\`` : ''}`),
      '',
    ];
    fs.writeFileSync(path.join(__dirname, 'docs', 'POS_ARRAY_COERCIONS.md'), posLines.join('\n'), 'utf8');
  } else {
    const stalePosPath = path.join(__dirname, 'docs', 'POS_ARRAY_COERCIONS.md');
    if (fs.existsSync(stalePosPath)) fs.unlinkSync(stalePosPath);
  }
...
export { pickPrimary, finalizeDictionary, pickPrimaryNoVerifiedCandidate, normalizeFile, posArrayCoercions };