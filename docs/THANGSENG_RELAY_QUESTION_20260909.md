...
**Question:** Does a common-noun object drop `-ko` before `ska`/`skenga` while a pronoun object keeps it? Or is this incidental to these two sentences specifically?

## Instruction to whoever answers
Please do not "correct" any of the above from general reasoning — direct native judgment on each numbered item is what's needed. This batch is deliberately held for a dedicated session per Project Owner's 2026-09-08 handoff (`ska_skenga` status: `NARROW_OPEN_USAGE_QUESTION`, "Do not mass-correct").
