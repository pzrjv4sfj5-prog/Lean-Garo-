...
| AI-002 | `assembleGrammar`'s object-extraction loop (`grammarEngine.js:544-545`) resolves a multi-word object phrase by falling back to `lookupGaro(lastWord)` alone when the full-phrase lookup fails — it never checks whether an *earlier* word in that phrase is the one that's actually unresolved. When it is, and the last word happens to resolve on its own (e.g. a trailing time adverb), that unrelated resolved word is silently placed in the object slot with the object marker, and the true unresolved word disappears with zero trace | silent wrong-substitution (worse than a drop — evades the existing `result.includes('[UNKNOWN]')` safety check at `sentenceBuilder.js:314` entirely, since no `[UNKNOWN]` string is ever produced) | **OPEN — discovered, not fixed** (found 2026-08-24 while building `src/research/detectUnresolved.js`, see `docs/CLAUDE_B_AI_FALLBACK_DESIGN_20260824.md` §1) | 1 confirmed live repro: `"i bought a gadget yesterday"` → `"Anga mejal·ko breaha"` (`"mejal"`="yesterday" wrongly takes the object slot, `"gadget"` vanishes) | Track per-word resolution status for every word in `objectWords`, not just the combined phrase + last-word fallback; if any individual word in the object phrase fails to resolve, the object should surface `[UNKNOWN]` (as the existing `!existingFullPhrase` case already correctly does for a single-word object) rather than silently accepting whatever the last word happens to resolve to. Engineering-only — propagates a signal the code already computes per-word during lookup, invents no new Garo content |
