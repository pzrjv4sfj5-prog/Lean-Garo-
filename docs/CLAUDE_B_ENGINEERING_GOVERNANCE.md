...
| AI-003 | `VERB_LEMMAS` (`lookupEngine.js`) is built from every `"to X"` dictionary key with no length cap — `X` can be a single word or an entire OCR'd print-dictionary gloss sentence. But every consumer (`sentenceBuilder.js`'s `lemmaSignal`, `grammarEngine.js`'s NP-subject-coherence check and infinitive-purpose check) matches it against exactly **one** tokenized English word at a time (with `ing`/`ed`/`s` stripped). Any lemma of 2+ words can therefore never match at any call site — not a partial/degraded match, a **total** one. When the sentence's other word(s) happen to independently exist as unrelated dictionary/`corrections.json` entries, this doesn't surface as `[UNKNOWN]`; it silently ships a confident, wrong, unrelated translation | new mechanism — total lookup failure for multi-word lemmas, masked by confident wrong output from unrelated single-word matches on the remaining tokens (distinct from AI-001's tie/no-candidate shapes and AI-002's last-word-wins substitution — no candidate competition happens here, the multi-word lemma is invisible to the matcher entirely) | **OPEN — investigated 2026-09-23 (Claude B), not fixed this session** | Live repro, `"to crumble down"`/`Be·rurua`, the held item named in the 2026-09-23 migration doc §4: `"it crumbled down"` → `"Ua ka·ma·ko"` (`"crumbled"` matches nothing, silently dropped; `"down"` independently matches `corrections.json`'s unrelated `"down"→"Ka·ma"`, — a directional/locative sense, not this verb's particle — and gets confidently shipped in its place, with no `[UNKNOWN]` anywhere to flag the failure). `"the wall crumbles down"` degrades further, to `"[UNKNOWN] de·wal [UNKNOWN] Ka·ma"`. Scope, measured this session: of 955 total `VERB_LEMMAS` entries, **609 (64%) are multi-word** and therefore invisible to every consumer as-is. Word-count distribution of those 609 is NOT the clean "verb + particle" shape the `"crumble down"` name suggests: 165 are 2 words, 129 are 3, and the tail runs all the way to 20-29-word entries (full defining sentences promoted as `"to X"` headwords, e.g. `"to seek a person's favour for another person"`) — most of the 609 are OCR-gloss headwords for exact-phrase lookup only, not genuine compositional phrasal verbs, and naively pattern-matching sentences against all of them (rather than just true short phrasal verbs) risks a new, worse failure mode: coincidental word-sequence matches inside unrelated sentences. | **Not designed yet — recommendation only.** Any fix needs a length cutoff decided BEFORE touching matcher code, not after: restricting multi-word lemma matching to the ≤2 (optionally ≤3) word band (166/609, the genuine particle-verb shape) is a defensible engineering-only cutoff (structural, not a linguistic judgment about any individual word); treating longer entries as compositional-verb candidates at all is very likely the wrong call regardless of matcher design and probably needs Claude A/D input on whether those long-gloss `"to X"` rows should even continue feeding `VERB_LEMMAS`, not just a matcher fix. Whoever picks this up next: do the cutoff-scoping call first, then (only for the ≤2/≤3-word band) extend `lemmaSignal`-style detection from single-token to sliding n-gram matching, and re-run the full suite (458 tests at time of writing) before shipping — this is exactly the kind of change most likely to regress unrelated sentences silently, per this row's own failure shape. |
