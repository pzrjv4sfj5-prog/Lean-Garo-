...
**Examples:** `Na·a Cha·ahama?` = "Did you eat?" (corrects draft `Na·a Cha·aha ma?`); `Na·a Bajal Re·angama?` = "Did you go to market?" (corrects draft `Na·a Bajal Re·anga ma?`); `Na·a mi cha·jokma?` = "Did you have lunch?" (corrects draft `Na·a mi cha·jok ma?`). **Repo-wide sweep closed 2026-08-28** (13 `master_dictionary.json` + 82 `corrections.json` violations fixed, per `.ai/WORKSTATE.yaml`) — the wider pre-existing corpus is now also correct, live-reverified 2026-08-31: `"are you eating?"`→`Na·a Cha·engama?`, `"is there rice?"`→`Mi dongama?`, `"have you eaten breakfast?"`→`Na·a nastha cha·ahama?` (all joined, no space).
**Counterexamples:** none currently live — the space/period-before-`ma` shape that used to appear in these three sentences (see prior revision of this rule) is now corrected project-wide; retained here only as a historical note, not an open item.
...
**Validation Status:** Verified.
**Confidence:** High.
**Launch Priority:** P0 — CLOSED project-wide (2026-08-28 sweep + this session's live re-verification of the three previously-flagged counterexamples, 2026-08-31).
