...
| `ring` | drink | ❌ No (per raka table) — **but see conflict note** | Ringa, ring·aha (raka table says this is the noun "ring"'s possessive, not the verb suffix) | THANGSENG_RULES_LOOKUP.md L40; corrections.json (`i drank`, `i drank water`) | **Medium** — **evidence conflict**: `corrections.json`'s `sikenga` ("want") construction produces `ring·na` ("want to drink") **with raka**, contradicting the raka table's "no raka" finding for this root. Either the raka table is incomplete (this specific infinitive context wasn't tested when it was built), or `ring·na` in `corrections.json` is itself an error. Flagged, not resolved. |
...
| `sikeng` | want / need | ❌ No | sikenga (want/wants/need — used as a general modal-like construction: `[verb infinitive]-na sikenga`) | corrections.json (13+ entries — one of the most productively-attested patterns in the whole dictionary) | **High** for the `sikenga` construction itself; but see conflict notes above — several of the infinitive forms feeding into it (`ring·na`, `a·gan·na`, `pora·na`, `bi·a·na`, `re·ang·na`) show raka inconsistently relative to those same roots' plain forms elsewhere. |
...
## Part 1+2 Summary: Confirmed Raka-Inconsistency Cluster

Four different roots (`ring`, `agan`/`a·gan·`, `porai`/`pora·`, and by
extension the whole `sikenga`-infinitive construction) show raka
appearing in their `-na` infinitive form when the same root is
consistently raka-free in every other attested context. This is a single
underlying pattern, not four unrelated bugs — worth investigating as one
