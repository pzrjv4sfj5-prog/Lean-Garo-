...
| `always` | **REOPENED — do not re-close mechanically.** Originally fixed `phrase_maps.js["always"]` `Pangnan`→`pang·na` per the `SUPERSEDED`/`VERIFIED` tags in `master_dictionary.json`. Reverted same session after Project Owner relayed direct native input (`always = Pangnan`) contradicting those tags — see `docs/CLAUDE_B_HANDOFF_20260814_angry_raka_placement.md` for full detail. `phrase_maps.js["always"]` is back to `Pangnan` (its pre-session value); build gate reconfirmed green, allowlist count back to 292. Not re-closing this key until Claude A resolves the tag-vs-native conflict. |
