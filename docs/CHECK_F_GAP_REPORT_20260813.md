...
| `always` | **CLOSED 2026-08-14 (Owner direct resolution).** Project Owner directly confirmed `always=Pangnan`, overriding the 2026-08-01 audit's SUPERSEDED tag. `phrase_maps.js["always"]="Pangnan"` unchanged (already correct pre-session value). `master_dictionary.json`'s `Pangnan` entry re-tagged VERIFIED (Owner relay, 2026-08-14); the audit's preferred `jring·jring`/`pang·na` variants annotated DISPUTED rather than deleted (citation-discipline convention). `known_cross_source_conflicts.json` baseline unchanged for this key (compiled_dict still resolves to `pang·na` — compile pipeline doesn't apply confidence precedence, per longstanding note — but `corrections.json`/`phrase_maps.js` win at runtime, so this is inert). Full detail in `docs/CLAUDE_B_HANDOFF_20260814_angry_raka_placement.md`. |
| `answer` | **CLOSED 2026-08-14 (Owner direct resolution).** Project Owner clarified this was never a supersession: `Aganchakani = answer (noun)`, `aganchaka = to answer (verb)` — two distinct words, not competing forms of one word. `corrections.json`/`phrase_maps.js["answer"]="Aganchaka"` unchanged (already the correct verb-sense default). `master_dictionary.json` updated: `Aganchaka` re-tagged VERIFIED with `pos:"verb"`; `Aganchakani` tagged `pos:"noun"`, both citing the Owner relay. `compiled_dict.json` still resolves to a third UNVERIFIED variant (`a·gan·chak·a` — compile pipeline doesn't apply confidence precedence) but this is inert at runtime for the same reason as above. |
| `a dog bit me` | **CLOSED 2026-08-14 (Owner direct resolution) — engineering AND linguistic.** Two native relays converged on `Angko achak chikaha` as primary (word order `Angko achak`, verb ending `chikaha`). `corrections.json` updated for all three related keys (`a dog bit me`/`dog bit me`/`the dog bit me`, previously `Achak Angko chikaha`) to match. `master_dictionary.json`'s untagged legacy entry (`An·tangko achik chanjok`) updated to the same value and tagged VERIFIED (Owner relay, 2026-08-14). `compiled_dict.json` regenerated via `node prepare-data.js` and now agrees. Removed from `known_cross_source_conflicts.json` baseline (no longer a mismatch, so a future regression here will now correctly surface as NEW). |
| `are you sleeping` | **CLOSED 2026-08-14 (Owner direct resolution).** Owner provided `Na·a tusiengama?`. `corrections.json` updated (was `Na·a Tusienga ma?`, spacing/case only — same underlying phrase). `master_dictionary.json`'s entry corrected from a typo (`Na·a tuengama?`, dropped "si") to `Na·a tusiengama?` and tagged VERIFIED. `compiled_dict.json` regenerated and agrees. Removed from `known_cross_source_conflicts.json` baseline.
