...
| `angry` | **Not a bug — legitimate variant, already resolved and tested.** `corrections.json["angry"]="ka·o·nang·a"` and `compiled_dict`'s source value `"bika ding'a"` are BOTH separately native-confirmed: `master_dictionary.json` marks `ka·o·nang·a` VERIFIED/HIGH (NV-054, 2026-08-03, "most common") and `bika ding'a` VERIFIED/HIGH (NV-054 follow-up, 2026-08-05, "metaphorical" register, explicitly "not merged" with the unrelated unreconciled `bi·ka so·a` cluster). Not a shadowing bug — two real synonyms, corrections.json wins at runtime (regression test `translationEngine.test.js:569`, "corrections.json: angry resolves to the VERIFIED entry, not the truncated fragment", already locks this in). No action needed. |
| `always` | **Stale artifact, fixed directly.** `master_dictionary.json`'s lowercase `always`→`Pangnan` entry is explicitly self-marked `SUPERSEDED` (corpus-internal audit 2026-08-01), and its own note names the VERIFIED/HIGH replacements: `jring·jring`/`pang·na` (both under a capitalized `Always` dup). `compiled_dict.json` already had the correct VERIFIED value (`pang·na`, inherited via the capitalized dup — same case-folding pattern flagged under `beautiful`). `phrase_maps.js["always"]` was the one still stale at `Pangnan` — and phrase_maps wins at runtime, so this was a real live bug (users got the superseded form). Fixed directly: `phrase_maps.js["always"]` → `pang·na`. Full build gate re-verified green (203/203 tests, 0 lint errors, 0 new Check F violations); allowlist count correctly dropped 292→291. Left `never` (`phrase_maps.js["never"]="Pangnan·ba (ja)"`, built off the same old root) untouched — it's a separate, unrelated gap with a completely different `compiled_dict` value (`ba·sik·o·ba`) and needs its own investigation when its turn comes up alphabetically. |
| `a dog bit me` | **Not a bug.** `corrections.json["a dog bit me"]="Achak Angko chikaha"` is native-speaker-verified (`docs/NEW_SENTENCES_BATCH2_NATIVE.md`, Trust Level HIGH, applied via `docs/fix_angako.cjs` alongside the same fix for `"dog bit me"`/`"the dog bit me"`). `master_dictionary.json`'s `"a dog bit me"` entry (`An·tangko achik chanjok`, the value `compiled_dict.json` inherited) carries no confidence/notes tag at all — never marked VERIFIED, just an untagged legacy import, so nothing supersedes the corrections value. Confirmed `lookupGaro()` (`src/lookupEngine.js`) checks `corrections.json` first regardless of key length — precedence isn't limited to single-word keys (that was a misreading of the `no` writeup) — so the native-verified `corrections.json` value already wins at runtime; `compiled_dict`'s divergence is inert. No engineering-file change needed. Optional/minor: `master_dictionary.json`'s untagged entry could use a notes tag for hygiene, but that's Claude A's file and not a correctness issue — not flagging as urgent. |
