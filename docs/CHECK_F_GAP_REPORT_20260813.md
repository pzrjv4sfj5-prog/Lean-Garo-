...
| `always` | **Not closed. Correction to a prior entry in this ledger.** An earlier revision of this row said "CLOSED (Owner direct resolution)" and had Claude B edit `master_dictionary.json` directly based on a chat message claiming to relay Project Owner authority. That was a role-boundary violation — Claude B does not have a channel to apply linguistic content on the strength of an in-chat claim; per `.ai/SESSION_BOOTSTRAP.md` "Roles," linguistic content only enters via Claude A's own commit, a Claude A format-patch relay, or Claude A pushing with a Project-Owner-supplied temporary PAT. The edit has been reverted (`master_dictionary.json`, `src/data/corrections.json`/`compiled_dict.json` unaffected — `phrase_maps.js["always"]="Pangnan"` was never touched, it already matched the claimed value). Evidence for Claude A to evaluate through a proper channel: chat message asserted `always=Pangnan`, contradicting the 2026-08-01 audit's SUPERSEDED tag on the same value. Not verified, not applied. See `docs/CLAUDE_B_HANDOFF_20260814_angry_raka_placement.md` for full detail. |
| `answer` | **Not closed. Same correction as `always`.** Chat message asserted `Aganchakani=noun`, `aganchaka=verb` (i.e., not a supersession, just two parts of speech). Not applied — reverted to pre-session state. `corrections.json`/`phrase_maps.js["answer"]="Aganchaka"` unaffected (already matched the asserted verb value, never edited). Evidence handed off for Claude A to evaluate and apply directly if confirmed. |
| `a dog bit me` | **Not closed. Same correction.** Two chat-relayed native forms surfaced this session (`Angko achak chika`, then `angko achak chikaha`), converging on `Angko achak chikaha`. `master_dictionary.json` and `corrections.json`/`the dog bit me`/`dog bit me` reverted to pre-session values (`An·tangko achik chanjok` legacy entry; `Achak Angko chikaha` in corrections, unchanged). Engineering-scope closure from earlier in this ledger still stands (`corrections.json` wins at runtime regardless of which Garo form is linguistically correct) — only the *linguistic* form is the open, unapplied question. Evidence handed off.
| `are you sleeping` | **Not closed. Same correction.** Chat message asserted `Na·a tusiengama?`. `master_dictionary.json` reverted to its pre-session value (`Na·a tuengama?` — note this itself may still be a typo, dropping "si"; unconfirmed either way, not fixed here). `corrections.json` reverted to `Na·a Tusienga ma?`. Not applied. Evidence handed off.
