...
| `bought` | **Correction (2026-07-10): genuinely conflicting, not resolved — escalate** | My original verdict here (`breaha`, citing the primary-source "she bought three books" sentence) missed a third data point: `docs/VALIDATION_CORPUS.md` already has `"have you bought"`→`"Na·a Bre·ajok ma?"` (**with** raka), marked `Verified`/`High`/`"raka-audit corrected"` — meaning a deliberate prior audit pass already chose the raka form. Two independently-credible sources now disagree (primary-source transcript vs. an already-audited corpus row), not one clear answer vs. one outlier. Retracting the earlier verdict — this needs Thangseng, not a repository-evidence call. |
