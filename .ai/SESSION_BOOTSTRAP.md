...
on arrival. Zero-runtime session (no engine code touched) — market=bajal
canonical cleanup (Owner directive, deleted Anti/ha·ti synonym rows),
closed skin/cow's skin corrections.json override + "Ritchasa gni"=102 open
item (Owner directive), closed NV-121/NV-158 -aha/-a tension as free
variation (Claude A linguistic analysis), adjudicated §6.2 for Claude B
(narrowed after Claude B's own concurrent An·chingko find — corrected same
day), drafted docs/THANGSENG_RELAY_QUESTION_20260920C.md (3 doubt-driven
probes, NOT sent). Rebased clean through 5 concurrent Claude B commits, no
file overlap. Gate green throughout: 8824/8824, 9/9, 456/456, 0 new
violations. Final HEAD 7ca6810. Full detail in the migration doc above.)
Prior: 2026-09-20 by Claude A
...
`-rang` plural-marking scope ruling, see "Current joint work package" below). Prior: 2026-08-23D by Claude A (session close —
sorry-interjection follow-up + a resume-protocol addendum for future Claude A sessions, see
...
read `docs/CLAUDE_A_SESSION_MIGRATION_20260823.md` first — a
...
still win if anything's stale by the time you read it. Top-priority
open item in that doc: the `yes` = Oe/Am/Hoe three-way (Am is what's
actually shipping at runtime today, Oe is UNVERIFIED per its own
confidence field — not VERIFIED as earlier docs claimed — and Hoe
is the other relay-confirmed candidate; the real disagreement is
Am vs Hoe, still not reconciled).
...
11. **Check compositional grammar before relaying a multi-word phrase
    ...
    often productive grammar composition — a verified possessive
    prefix or suffix applied to an independently-verified root — not
    idioms that need their own native confirmation. Before adding a
    multi-word phrase to a relay batch: check whether the engine's
    existing compositional rules (`POSSESSIVES`/`sov-assembly` in
    `grammarEngine.js`, tense/aspect markers, imperative suffixes,
    classifier+number formulas) already cover it from pieces that are
    each independently VERIFIED/HIGH elsewhere in the corpus. If so,
    close it directly (cite the composing pieces in the master row's
    notes, no NV number needed — it's corpus-internal reasoning, not
    new native evidence). Only relay what's genuinely irreducible:
    single-word vocabulary with no cross-corpus support, or phrases
    where the compositional pieces themselves conflict or are
    uncited. (See NV-083/088's "my dog"/"my house" for the original
    finding, and `docs/CLAUDE_A_SESSION_MIGRATION_20260820c.md` for
    the first batch resolved this way: "my father", "he has eaten",
    "sit down", "a tree".)

12. **Apostrophe-preserving lookup spot-check** (Project Owner
    directive via Claude A, 2026-08-20c). `PHRASE_MAPS` keys may
    contain apostrophes intact (e.g. `"i don't know"`), but a naive
    lookup pipeline can silently strip apostrophes before checking
    against those keys — this exact bug made 13 keys unreachable in
    production, worst case "i don't know" shipping as `Anga uia.`
    ("I know" — polarity fully reversed) instead of `Anga uija`. Any
    future change to `translationEngine.js`'s lookup/normalization
    pipeline (`lookupPhrase`, `lookupGaro`, or any new cascade stage)
    must be spot-checked live via `translate()` against at least one
    apostrophe-containing key afterward — a passing test suite alone
...
**NEW, 2026-08-31, Claude B — DATA CLEANUP handed off to Claude A, no engineering change.** Full doc: `docs/CLAUDE_B_SESSION_MIGRATION_20260831.md`. Re-verified the `bye`/`bland`×2 `master_dictionary.json` rows flagged 2026-08-30 (notes say `SUPERSEDED`, `confidence` still `unverified`) are still present and still have **zero runtime impact** — `prepare-data.js`'s `notesDeclareSuperseded` fallback correctly excludes all 3 from `src/compiled_dict.json`/`compiled_dict_alternates.json` regardless of the stale field. Explicitly handed to Claude A as a data/citation-truth correction (update `confidence` to `superseded` after checking provenance) — see `.ai/WORKSTATE.yaml` `claude_a.pending_data_cleanup_from_b` for exact rows and line numbers. Claude B has no write authority over `confidence` and made no data edits. Not urgent, no live bug — flagged so it isn't silently carried forward indefinitely.

**NEW, 2026-08-28, Claude A — NV-097/NV-098, CLEAN CLOSE.** Full doc: `docs/CLAUDE_A_SESSION_MIGRATION_20260828.md`. **NV-097** processed an uploaded Thangseng PDF transcript batch (Rakka-normalized: apostrophe/curly-apostrophe as morpheme separator → `·`, hyphen as English/Garo suffix separator → `·`), closing 10 of the ~14 items open since 2026-08-26 (`don't eat`, `don't go`, `give me water`, `go away`, `help me`, `how are you`, `how many`, `how much`, `how much is this`, `hurry up` — new VERIFIED/HIGH, stale unverified rows superseded where directly contradicted), reconfirming 3 (`bring (imperative)`, `happy`, `how (alt)`), and logging 6 new flagged variants deliberately NOT reconciled against existing primaries (`dance (alt)`, `eaten` ×2, `knowledge (alt, fuller compound)`, `live (alt)`, `living`). 4 propagation-gap fixes synced `corrections.json`/`phrase_maps.js` to match (also fixed one live RULE-046 space-before-`ma` violation in `how are you`). One self-caught mistake (a speculative bare `hurry` row) was found and reverted before commit — see the migration doc's "Do NOT repeat" section. **NV-098**, same session, resolved NV-097's one flagged contradiction: Thangseng directly confirmed `uija`/`ma·sija` are synonyms for "i don't know", not competing candidates — both promoted to VERIFIED/HIGH, `uija` kept primary. **Only `go` and `only` remain open** from the original ~14-item count — both need an explicit future Thangseng relay question or commit archaeology, not more corpus-internal investigation. Every touched key live-verified via `translate()`. Gate green throughout: 247/247 tests, 8187/8187 dictionary entries, 0 new `repository-intelligence.js` violations.

**NEW, 2026-08-26, Claude A — Thangseng relay batch worked (MID-TASK, not complete) + ska/skenga closed.** Full doc + Resume Protocol: `docs/CLAUDE_A_SESSION_MIGRATION_20260826.md`. **Relay batch identity corrected:** the "138-item" count carried since NV-088 (2026-08-20) was a frozen historical snapshot, never updated as items closed — actual source (`docs/THANGSENG_RELAY_BATCH_20260820.md`) has **149 items** (91 Part A + 58 Part B). **Ska/skenga closed** on new Thangseng evidence relayed this session (`ska`=simple want, `skenga`=continuous of `ska` — not `sikenga`=continuous of `sika`/push-insert, which 8 "I want to X" phrase entries had regressed to): all 8 fixed in `master_dictionary.json`/`corrections.json`, verified via `translate()`, legitimate water/food `skenga` left untouched. **7 engineering-propagation-gap fixes** (master had native-confirmed VERIFIED/HIGH, `corrections.json`/`phrase_maps.js` never synced — same bug shape as ska/skenga): `bring`, `choose`, `land`, `it is not good`, `they are working`, `thank you very much`, `you did well`. **1 citation-hygiene fix** (`cooked`=`min·a` superseded per NV-095's later reconciliation, runtime already correct). **3 items applied from NV-092 evidence that was documented but never actually written to `master_dictionary.json`** (confirmed via `git show e515d14` — genuinely never committed, not lost): `stand up`, `take revenge`, `to spread`. **STOPPED before finishing the full 149-item accounting**, per explicit Project Owner instruction rather than guessing: discovered NV-093's commit (`9ef4603`) wrote several items under parenthetical-suffixed keys (`"begin (infinitive)"`, `"bland (alt)"`, `"bye (clarified)"`, `"coming (progressive)"`, and ~8 others) rather than the bare relay-batch keys — nothing lost, but whether these parenthetical rows correctly answer the bare-key lookup or represent a genuinely distinct sense is an open POS/sense judgment call needing evidence, not assumption. **Next Claude A session: resolve this first**, per the migration doc's Resume Protocol, before continuing the relay batch. Gate green throughout, every commit individually verified: 235/235 tests, 0 new `repository-intelligence.js` violations.

**NEW, 2026-08-25, Claude B — AI-002 fixed (engineering-only).** Full doc: `docs/CLAUDE_B_SESSION_MIGRATION_20260825.md`. `analyzeGrammar`'s object-resolution fallback (`src/grammarEngine.js`) checked only the LAST word of a multi-word object phrase for resolution; if an earlier word was the one that actually failed and the last word (e.g. a trailing time adverb) happened to resolve on its own, that unrelated resolved word was silently placed in the object slot with zero `[UNKNOWN]` trace — confirmed live repro `"i bought a gadget yesterday"` → was `"Anga mejal·ko breaha"`. Fixed: every object word is now checked individually, surfacing `[UNKNOWN]` if any fails to resolve, but only when a finite verb exists elsewhere in the sentence — a first attempt without that gate regressed the existing "I am lying in bed"/"I am lying down" tests (a structurally distinct no-finite-verb copula construction where a stray unresolved word is expected to be dropped), root-caused and corrected before landing. `existingFullPhrase` priority unchanged. 6 new regression tests added; final gate 235/235 tests pass, 0 new `repository-intelligence.js` violations. `docs/CLAUDE_B_ENGINEERING_GOVERNANCE.md` §4's AI-002 row updated to **FIXED**. AI-fallback prototype untouched this session, still the standing next task; linguistic backlog untouched.

**NEW, 2026-08-25, Claude A — `-rang` plural-marking scope ruling (closes the item handed off in `docs/CLAUDE_B_RANG_PLURAL_AUDIT_20260824.md`).** Full ruling: `docs/CLAUDE_A_RANG_PLURAL_RULING_20260825.md`. **Ruling: status quo, formalized — `-rang` is used only where explicitly native-confirmed, not ruled productive for any noun class, universal or restricted.** Only 3 native-confirmed data points exist (children/animate, fruits/inanimate-count, coins/inanimate-count) — enough to rule out a strict animate-only hypothesis, not enough to establish universal or class-restricted productivity. A broader ruling in either direction would mean generating guessed `-rang` forms with no individual native confirmation, or inventing an unconfirmed noun-class signal to gate them — both are exactly the engineering-invents-linguistic-content move evidence-first discipline exists to prevent. **This is deliberate, documented policy, not an unaddressed gap** — a future session finding "dog"/"dogs" both ship the identical bare form should not reopen this as a suspected bug. Zero engineering change, per Claude B's own audit (§7): the three existing `-rang` forms keep shipping via their dictionary rows, every other bare plural correctly falls through to the unmarked singular. Flagged as an open relay question for a future Thangseng batch (not yet sent): does `-rang` generalize to other count nouns — more data points needed before any productivity ruling, in either direction. No dictionary/engine content touched.

**NEW, 2026-08-24C, Claude B — migration close, no new engineering/linguistic work (Rule 9).** Full doc + resume protocol: `docs/CLAUDE_B_SESSION_MIGRATION_20260824C.md`. Verified no runtime errors (229/229 tests, 0 gate violations, new `src/research/demo.js` prototype re-run clean) and nothing local-only (fresh `git fetch`/`status`, found and pulled 2 new Claude A commits first). Logged last session's discovered `assembleGrammar` object-loop wrong-substitution bug as **AI-002** in `docs/CLAUDE_B_ENGINEERING_GOVERNANCE.md` §4's standing architectural-investigation table, so it's tracked going forward rather than living only in a dated migration doc. Migration doc's Resume Protocol section spells out for the next Claude B: Rule 13 (`docs/CLAUDE_B_ENGINEERING_GOVERNANCE.md`) is mandatory reading every session, not just first bootstrap; resync-before-work is non-negotiable; if continuing the AI-fallback prototype, no research result may ever reach `master_dictionary.json`/`corrections.json`/`compiled_dict.json` directly and `STATUS.CONFIRMED` must not be added to `researchFallback.js` without explicit Project Owner sign-off; if not continuing it, AI-002 is independently pickup-able and already scoped.

**NEW, 2026-08-24B, Claude B — AI/web fallback research prototype designed and built as a standalone, engineering-only layer; not wired into the translator.** Full report: `docs/CLAUDE_B_AI_FALLBACK_DESIGN_20260824.md`. Per explicit Project Owner brief: audited the existing cascade first, found two independent silent-loss mechanisms that evade the existing `[UNKNOWN]`-string safety checks entirely — `assembleSentenceSOV`'s `.filter(p => p.garo)` silently drops unresolved words with zero marker, and `assembleGrammar`'s object loop can silently substitute a *different*, wrongly-resolved word into the object slot when a multi-word object phrase's true unresolved word isn't the last one (confirmed live on both: `"widget"` vanishes cleanly; `"gadget"` gets replaced by `"yesterday"` in the object slot with zero trace). Built `src/research/detectUnresolved.js` (re-derives resolution status from original English words via the same lookup primitives the engine already uses — immune to both mechanisms above by construction) and `src/research/researchFallback.js` (the `researchMissingWord()` interface exactly as specced: provider-agnostic, in-memory cache, `STATUS` enum with no `CONFIRMED` value, `candidate_garo` stays `null` on disagreement). `src/research/demo.js` proves the pipeline end-to-end on 3 real cases using real web-search evidence gathered this session — correctly returns `NO_EVIDENCE_FOUND` rather than fabricating a candidate when no Garo-specific evidence exists (the honest, and more common, outcome for a low-resource language). Zero write path to any `*.json` dictionary file exists anywhere in the new module — verified by inspection, one of 4 independent structural barriers documented in the design doc's §6. Not wired into `translationEngine.js`; zero existing files touched; full gate re-verified byte-identical (229/229 tests, 0 new `repository-intelligence.js` violations). Discovered-but-out-of-scope: the `assembleGrammar` wrong-substitution bug is a genuine independent engineering bug flagged for a future dedicated session, not fixed here.

**NEW, 2026-08-24, Claude B — `-rang` plural-marking audited end-to-end per explicit Project Owner instruction, no ruling made; separate stale-override bug found and fixed.** Full report: `docs/CLAUDE_B_RANG_PLURAL_AUDIT_20260824.md`. Confirmed no code anywhere generates or strips a `-rang` suffix — the 3 existing VERIFIED/HIGH `-rang` plurals (children/`Bi·sarang`, fruits/`biterang`, coins/`tangka bisilrang`) are 100% hard-coded dictionary rows, each its own independent native-relay entry. Live-tested across animate/inanimate/count/mass nouns; the 3 confirmed cases span 1 animate + 2 inanimate nouns, contradicting a strict animate-only hypothesis but insufficient to establish universal productivity. Engineering consequence of each of the 3 possible rulings (universal/class-restricted/confirmed-only) laid out — none needs new architecture, all are additive at `assembleSentenceSOV`'s one existing fallback point. **Item (a) (plural-marking scope) remains open, handed to Claude A with evidence, not a guess.** Separately, found and fixed a genuine Rule 8 stale-override bug unrelated to the linguistic question: `corrections.json`'s `"coins"` shipped the singular value, shadowing master's own already-VERIFIED `tangka bisilrang` (NV-089) — pure sync fix, no new linguistic content, confirmed live, gate green (229/229 tests, 0 new `repository-intelligence.js` violations). Pushed `1505301`. Resolves 2 of Claude B's 3 flagged items from 2026-08-23C were already closed by Claude A's NV-095 (bland/tasteless, you=Nang/Na·a) — confirmed via clean resync before starting.

**NEW, 2026-08-23, Claude B — items 1/2/3/5 from the 2026-08-23B audit closed (all 4 ENGINEERING items); items 4/6/7 flagged for Claude A, not attempted.** Resumed via `docs/CLAUDE_B_SESSION_MIGRATION_20260823B.md`. Closed: (1) stale `corrections.json` overrides (song/telling/studying) synced to master VERIFIED values. (2) `parseCountingPhrase()` adjective-slot gap — `[NUMBER][ADJ][NOUN]` phrases now reach classifier composition (0.96) instead of `sov-assembly` (0.75); the function has no dictionary access, so it now exposes both the full-phrase candidate (tried first — genuine multi-word nouns like "sugar cane" unaffected) and a `nounOnly` fallback for the caller. **Self-correction, logged not hidden:** an earlier turn this same session incorrectly reported item 2 as already implemented and pushed when only investigation had happened — caught while writing this session's close record, actually implemented before closing. (3) `assembleSentenceSOV` fixed two bugs: sibilant-ending plurals ("boxes") silently vanished from output (`es$` fallback strip added); adjectives ending in `·a` were misclassified as verbs via an overly-broad suffix regex and stranded at the sentence tail — no POS data exists anywhere in this repo (confirmed RC-CANDIDATE-003) to distinguish adjectives from verbs, so fixed structurally instead of guessing: only the LAST verb-signal match is elected as the finite verb, earlier matches fall back to `nonVerbs` preserving original relative order (keeps adjectives adjacent to their noun with zero new linguistic claims). (4) item 5's remaining half (verb "see" never detected at all — bare-root Garo verbs like `Nia` carry no tense/mood suffix) fixed via new `VERB_LEMMAS` (`lookupEngine.js`), mechanically derived from the dictionary's own 939 `"to X"` headwords (Claude A/D's existing infinitive-marking convention, reused not invented) — a definitive lemma match takes priority over the ambiguous suffix regex whenever both exist in one sentence. Gate green throughout, every commit: 229/229 tests by close (up from 220), `repository-intelligence.js` PASSED 0 new violations every time, `compiled_dict.json` byte-identical (all fixes engine-only, zero linguistic-data edits beyond item 1's mechanical value sync). Two clean merges of concurrent Claude A pushes mid-session (NV-092/NV-093 batch + session close; then a Rule 8 `gossip` fix + `dist` rebuild) — no key overlap, gate re-verified green after each. **Flagged for Claude A, not attempted (DATA/LINGUISTIC, correctly out of Claude B scope):** (a) no productive plural rule — `dog`/`dogs` both ship the identical `Achak` string, needs a ruling on plural-marking scope before an engineering rule can be written; (b) `bland`/`tasteless` compiles to a corrupted fragment `·brok·` (bare raka marks) — master rows `UNVERIFIED`, needs a correct value supplied; (c) `you`=`Nang` in the dictionary but every runtime path ships `Na·a` instead — could be a legitimate nominative/oblique case distinction, not asserted as a defect, needs confirmation either way. Full detail: `docs/CLAUDE_B_SESSION_MIGRATION_20260823C.md`.

**NEW, 2026-08-23, Claude A — NV-092 (41-item batch) + NV-093 (48-item batch): Project Owner chat-relay batches processed, backbone/waist runtime bug fixed, WORKSTATE.yaml YAML-integrity fixes.** Both batches checked item-by-item against current repo state before writing anything — most items in each were exact reconfirmations of already-VERIFIED data, no action. NV-092 closed 2 previously-flagged priority items (stand up=`chadengbo` imperative, take revenge=`a·jak soka`) and 2 items left OPEN at NV-089 close (they are working=`Uamang kam ka·enga`, it is not good=`(iade) namja` — **though corrections.json ships a third, unreconciled form for both, not force-fixed**), extended the flagged Anga/Ango possessive-existential pattern 2 more data points, added 4 coexisting alt-forms, closed a my-mother duplicate-representation gap, fixed 2 clean Rule-8 stale overrides. Flagged, not resolved: i understand (uia vs ma·sia), let's drink/eat (-ha VERIFIED vs this relay's -na, which fits the let's-play/sit/work paradigm better), skenga/sikenga paradigm, i-want-to-work root choice. NV-093 promoted 10 previously-unverified rows to VERIFIED/HIGH, added 14 new coexisting forms/senses, and found a genuine data-entry bug unrelated to any native-data judgment call: `corrections.json`'s `"backbone": "kangkare"` was shipping the *waist* translation under the backbone key — fixed. Caught and removed 2 duplicate rows before commit (help noun/verb already existed from a prior session, missed on first grep); confirmed the 10 other exact-duplicate rows in the corpus predate this session (already a separate flagged backlog item from NV-089 close). Also fixed a stale `WORKSTATE.yaml` head pointer on resume (bookkeeping only, no repo content change), and 3 YAML-breaking unclosed-quote bugs across this session's own commits and one in Claude B's concurrent `548cc61` — all found while validating a clean merge with Claude B's audit-only session (no key overlap). Gate green throughout every commit: 220/220 tests, 0 new repository-intelligence violations. Pushed `e515d14`, `fac66bd`, `d89d5da`, `9ef4603` (migration doc + WORKSTATE.yaml close still to follow, per usual pattern). Full detail: `docs/THANGSENG_NATIVE_VALIDATION.md` (NV-092/NV-093 entries) and `docs/CLAUDE_A_SESSION_MIGRATION_20260823.md`.

**NEW, 2026-08-22 (session C), Claude A — NV-089/089b/090/091: full `THANGSENG_RELAY_TABLE_20260821B.md` processed.** Claude C's ~150-row relay table (2026-08-21 WhatsApp transcript) closed across 4 sub-batches as the Project Owner fed in more direct-relay data through the session. 14 UNVERIFIED→VERIFIED promotions, ~40+ new/cited VERIFIED entries, several coexisting alt-forms added (not superseding uncited existing candidates without stronger evidence), 3 explicitly-rejected constructions replaced, 2 sense-splits (dead, dried). Tied-candidate judgment calls: elephant (mong·ma primary), outside (A·pal un-superseded, defecate-collocation caveat). Closed the live `translate("king")` bug at the citation level (Raja VERIFIED/HIGH; Claude B's structural `pickPrimary` fix still needed for durable closure). `you`=`Na·a` directly confirmed (Nang flagged, not superseded, likely a case-form). Fixed 4 genuinely stale runtime overrides across `corrections.json`/`phrase_maps.js` per Rule 8 (goodbye, i am sad, hurry, stand). **Key open finding: `yes` is now a three-way** — `Am` is what `translate("yes")` actually ships (pre-existing `corrections.json` override, predates this session), `Oe` is master's own VERIFIED tag, `Hoe` is a third relay-confirmed candidate used sentence-internally elsewhere in the corpus. Not resolved — top priority for the next session. Also still open from NV-089, untouched since: i understand (vs NV-087), sit/stay (vs NV-080, part of a live/stay/sit consolidation question), let's drink/eat (vs NV-083), skenga/sikenga paradigm, they are working, it is not good. `answer the question`/`i have a question` deliberately held (tied to the open `answer` pickPrimary tie-break and the Angna/Ango pattern respectively) per explicit Project Owner confirmation. Two grammar-rule candidates flagged, not built: you=Na·a/Nang case paradigm, Anga/Ango possessive-existential pattern (4+ data points, likely ready to become a real rule next session). Duplicate-representation check: 0 new duplicates introduced, 10 pre-existing exact-duplicate rows found and flagged separately (unrelated to this work). Full runtime-error-sweep: 14,566 `translate()` calls, 0 errors. Gate green throughout: 8159 entries, 9/9 grammatical corrections, 220/220 unit tests, 0 new repository-intelligence violations, including after a clean mid-session rebase against 4 concurrent Claude B commits (docs-only, no conflict). Pushed `52355ca`, `f553017`, `2503fe3`, `e1864b4`, `ac8fefe`, `13d951c` (migration doc + WORKSTATE.yaml close still to follow). Full detail: `docs/THANGSENG_NATIVE_VALIDATION.md` (NV-089/089b/090/091 entries) and `docs/CLAUDE_A_SESSION_MIGRATION_20260822C.md`.

**NEW, 2026-08-22, Claude A — Project Owner-directed Role Self-Audit + permanent operating governance established.** Audit (`docs/CLAUDE_A_ROLE_SELF_AUDIT_20260822.md`) concluded ROLE HEALTH: NEEDS CORRECTION — zero new grammar rules since RULE-045 (2026-08-02) against 43 NV closures in the same window; the 410-entry classifier-derivation backlog (stalled since 2026-08-09, per the standing discipline that mechanical derivations from an already-confirmed formula still require one-by-one native confirmation) is the clearest concrete symptom. Core discipline (evidence-first, citation-honest, role-boundary-respecting) found intact and not at risk. Resulting permanent governance: `.ai/CLAUDE_A_OPERATING_GOVERNANCE.md`, **mandatory reading before any linguistic work, every session**. Key changes: mandatory A/B/C/D/E classification of every incoming item before relay or fix; native evidence sought only for genuinely unresolved questions, not as a default; new `DERIVED` confidence tag (distinct from `VERIFIED/HIGH`) for rule-derived entries, must cite the licensing rule + a representative validated example; a rule-generalization drift self-check (3 consecutive vocabulary-only sessions with no new/updated rule = explicit flag to the Project Owner); an explicit classifier/number derivation sequence (rule → representative spot-check → systematic derivation → engineering propagation → QA) — establishes the *method* for the 410-entry backlog, does **not** resolve it this session, per direct instruction; mandatory duplicate-representation check with an explicit PASS/gap statement per closure; mandatory extended Runtime Handoff section or explicit "Runtime Handoff: None."; mandatory POS/sense-default statement whenever superseding a conflicting row (the unresolved "answer" Aganchaka/Aganchakani tie, open since NV-077, is the named cautionary example). Session was governance/documentation only — no dictionary, `corrections.json`, `phrase_maps.js`, or compiled data touched; gate verified green 3x (8132/8132, 218/218, 0 new repository-intelligence violations), including after two mid-session rebases (one against unrelated Claude B/C work, one against Claude B's own concurrent parallel engineering-governance work touching the same two files — `.ai/SESSION_BOOTSTRAP.md`'s header conflicted and was resolved keeping both roles' governance pointers, nothing dropped). Pushed `0c1c721`, then `86ffb26` (post-rebase). Full detail: `docs/CLAUDE_A_SESSION_MIGRATION_20260822.md`.

**NEW, 2026-08-20c, Claude A — NV-086/087/088 (real-time WhatsApp relay closures) + a critical corpus-internal runtime bugfix + 3 override-vs-master fixes + 4 compositional relay-batch closures.** NV-086: "did you go to market" confirmed (`Na·a bajalchi re·angama?`), fixed a stale wrong-tense `corrections.json` override. NV-087: Anti/week framework, 4 of 5 questions closed (`week`=`Anti`, `a week`=`Antisa`, `two weeks`=`Antigni`, `next week`=`Mikkang anti`, `antio` confirmed same root via the `-o` locative) — flipped a legacy untraceable `sop·ta` "VERIFIED/HIGH" tag to SUPERSEDED in favor of real native evidence; **"three weeks" was NOT answered, stays open**; also fixed "i understand" (wrong shipping override). NV-088: "my house" confirmed (`ang·ni nok`), master held a different uncited value — SUPERSEDED, new VERIFIED/HIGH row added. **Methodology correction from the Project Owner:** "my X" possessive phrases (my dog/my house/my father) are productive grammar composition (`POSSESSIVES["my"]="Angni"` + noun-root, via the engine's `sov-assembly` method) — not idioms needing individual native relay confirmation. Verified true (e.g. "my table"/"my pen" already compose correctly, never hardcoded). **Standing rule:** before relaying a multi-word phrase to native, check whether existing compositional grammar already covers it from independently-verified pieces; only relay genuinely irreducible vocabulary/constructions. (Formally added as Rule 11, 2026-08-22 — see rules list above.) **Critical runtime bug found and fixed:** `translationEngine.js`'s phrase-map lookup stripped apostrophes before checking `PHRASE_MAPS` keys, which contain apostrophes intact — 13 keys silently unreachable. Worst case: "i don't know" shipped as `Anga uia.` (= "I know", **polarity fully reversed**) instead of the correct `Anga uija`. Also: wrong verb root ("i don't understand"→"i don't know"'s value, "i don't want"), wrong construction ("don't be afraid" — statement instead of imperative), word-salad output ("don't give up", "don't forget your language"). Fixed: lookup now tries `lowerWithApos` → `cleaned` → `lower`, matching the pattern already used for the corrections lookup one line above. This exposed 3 pre-existing **override-vs-master conflicts**, all fixed from existing master VERIFIED citations (no native input needed): "i don't understand" (one of the 20 conflicts Claude B flagged in `CLAUDE_B_HANDOFF_20260819` section 3, previously unreachable, shipping wrong value once the bugfix made it reachable — corrected to master's VERIFIED target `Anga man·ja`); "what job do you do" (missing a syllable); "what is your name" (matched neither of master's two VERIFIED variants). Also closed 4 relay-batch items **compositionally**, no native input needed: "my father"=`ang·ni ba·ba` (also fixed a raka-dot spelling bug), "he has eaten"=`Ua cha·jok`, "sit down"=`Asongbo` (also fixed a stale `phrase_maps.js`/`corrections.json` duplicate with mismatched punctuation), "a tree"=`Bol pangsa` (already correct, cited). Gate green throughout: 8132 entries, 9/9 grammatical corrections, 218/218 unit tests, 14532/14532 runtime-sweep, 0 errors, 0 new repository-intelligence violations. Pushed `7b6d567`, `1d953cb`, `df6f83e`, `56cbae0` (migration doc). **Still open: 138 relay-batch items** — the closures above used existing corpus evidence, not new relay answers, so the native-relay-required count is unchanged in content (89 Part A single words + ~39 Part B phrases). **4 flagged priority for the next relay batch** (compositional pieces conflict or are uncited, blocking otherwise-composable phrases): "give me water" (imperative give-stem `on-` conflicts with verified root `ron-`), "stand"/"stand up" (two competing uncited candidates, no verified base), "take revenge" (only a noun-form citation exists, verb-phrase form diverges), "self" (atomic root, zero corpus cross-check). Plus "three weeks" (Anti/week Q3, unanswered). Full detail: `docs/CLAUDE_A_SESSION_MIGRATION_20260820c.md`.

**NEW, 2026-08-20, Claude A — NV-083: closed citation gaps on 8 Thangseng-relay items (let's drink/eat/play/sit/work, my dog, beautiful, child, wait); caught a grammarOverrides byte-comparison trap before it shipped a runtime regression.** Resumed via `docs/CLAUDE_A_SESSION_MIGRATION_20260819...` chain; source this session was `docs/THANGSENG_RELAY_BATCH_20260820.md` (150-item batch, only a small subset answered so far — 144/150 still open). `let's play`/`let's sit`/`let's work`/`my dog` were not pure citation-only: `master_dictionary.json` held stale, unconfirmed values diverging from what `corrections.json` actually shipped at runtime (`Hai kalaha`→`Hai kalna`, `Hai asongha`→`Hai asongna`, `Hai dakha`→`Hai dakna`, `angni mang`→`ang·ni achak`) — old rows tagged SUPERSEDED, new VERIFIED/HIGH rows added citing NV-083, live runtime value unchanged (was already correct). Closing the `wait` citation gap first attempt added `"Damo / Sengbo"` (spaced) — semantically identical to but byte-different from the existing `grammarOverrides['wait']` constant `"Damo/Sengbo"` (no spaces) in `prepare-data.js`; pickPrimary treated the new spacing as a fresh VERIFIED selection, silently flipping the compiled runtime value and breaking `tests/unit/translationEngine.test.js:1117` (217/218, plus an 8/9 `test-dictionary.js` mismatch initially and incorrectly assumed pre-existing — it was the same cause). Caught by the gate before push, corrected to match the constant byte-for-byte, re-verified fully green. **Lesson carried forward for Claude A/B coordination:** any new VERIFIED/HIGH row for a key that already has a `grammarOverrides` entry needs byte-for-byte value comparison against that constant, not just same-meaning comparison, before commit — a real architectural coupling invisible from `master_dictionary.json` alone. Evidence-first held under direct pressure twice: declined a verdict-only "week=anti confirmed" claim without the actual relay text; declined "correct entries don't need Thangseng" pushback re: the 150-item no-VERIFIED-candidate batch (no citation isn't the same as wrong, but isn't confirmed either). Gate green: 8127 entries, 9/9 grammatical corrections (was 8/9 mid-session, traced and closed), 218/218 unit tests, 14523/14523 runtime-sweep, 0 errors. Pushed `72ff3a6`, then `d0f3eb8` (migration doc). WORKSTATE.yaml/SESSION_BOOTSTRAP.md were NOT updated in the same session as the fix commits (flagged outstanding at close) — done now, this resume, first action per Rule 10. **Still open, untouched:** 82 Brave, 94 Agree, 96 Bear (verb sense), 44 Gong (instrument sense), Anti/Antio/week (5 questions, unanswered), `THANGSENG_RELAY_BATCH_20260820.md` (144/150 open), Claude B's resync-sweep handoff (`bear` case/sense risk = same word as 96; `elephant`/`outside` tied-VERIFIED-candidate picks need a Claude A judgment call, not new relay evidence; 20 "override disagrees with VERIFIED but not tagged SUPERSEDED" items already carry citations, worth reviewing directly next session rather than relaying). Full detail: `docs/CLAUDE_A_SESSION_MIGRATION_20260820.md`.

**NEW, 2026-08-14, Claude B — counting-system engineering work from Claude C's independent audit closed end-to-end: runtime-propagation precedence bug fixed, SUPERSEDED-only-candidate pipeline hole closed, 190 keys stopped shipping already-rejected values, regression tests added, full gate green.** Resumed via `docs/CLAUDE_B_SESSION_MIGRATION_20260814_D.md` (checkpoint `ed53f47`); `git fetch` clean, zero drift. Claude C's audit (relayed live by the Project Owner in chat this session, not a committed repo file) found two distinct engineering bugs: **(1) §3.5, runtime propagation:** `translationEngine.js`'s own header docstring has always documented "2. Exact phrase match (compiled dict)" as outranking "5. Number + classifier engine", but `8f7dfba` wired classifier composition in at step 1.6, ahead of the exact-phrase lookup — inverting the documented cascade. Confirmed directly: `translate('twenty student')` returned the stale bare-noun-composed value even though `compiled_dict.json` already carried NV-073's fixed phrase-level entry, because classifier composition ran first and returned before exact-phrase ever got a turn. Fixed by moving the exact-phrase block ahead of classifier composition; composition remains the correct fallback for phrases with no dedicated `compiled_dict.json` entry (verified unaffected). **(2) §3, SUPERSEDED-only candidates (the "twenty students" case; also the un-actioned half of Claude A's 2026-08-13 audit's handoff item (a)):** `master_dictionary.json`'s only candidate for a key can be explicitly marked `SUPERSEDED` with no VERIFIED replacement, yet the pipeline still shipped that exact value whenever `garo_dictionary.json` (no `notes` field, can never carry a SUPERSEDED tag) independently duplicated the identical wrong string untagged — the same rejected content resurfacing from a file structurally unable to flag it, defeating the whole SUPERSEDED mechanism. Fixed in `prepare-data.js`: `normalizeFile()` now also returns which garo values were marked SUPERSEDED per key; `finalizeDictionary()` filters any non-master (`source !== 2`) candidate matching one out before `pickPrimary` sees it, scoped to non-master specifically so master's own legitimate same-string VERIFIED/SUPERSEDED coincidences (`two dogs`) are untouched. Keys left with no surviving candidate are held out of `compiled_dict.json`/`compiled_dict_alternates.json` and logged to `docs/SUPERSEDED_ONLY_KEYS.md` (auto-regenerated every build) instead of silently shipping known-wrong content. 190 keys held this run — the bulk of Claude A's ~230-key 2026-08-13 finding not already closed by that session's teacher/mountain/village/road/banana/car relay (house/rice/water/food chief among the remainder, still needing native input). Both fixes are pure engineering, zero `master_dictionary.json`/`garo_dictionary.json`/`corrections.json` edits. **Regression tests:** 3 new cases in `tests/unit/translationEngine.test.js` (exact-phrase wins, classifier still reached as fallback, plural defect no longer surfaces the rejected string) + new `tests/unit/prepare-data.test.js` (5 tests directly against `finalizeDictionary` with synthetic data: held/shipped/master-reconfirmed/unaffected-by-default). Updated 2 stale pre-fix expectations that had locked in bug behavior as correct (`one person`'s `expectMethod`; `rc037_bird_classifier.test.js`'s `two cars`, which now gets its own held-not-shipped assertion instead of asserting a compiled entry existed at all). **Flagged, not fixed (Claude A's territory):** `student`'s bare-noun dictionary entry was never updated to NV-073's `Chattro` root alongside the phrase-level fix, so `translate('twenty students')` (plural, no phrase-level entry) now falls through to classifier composition consistently rather than shipping an unrelated wrong bulk value, but still composes from the stale `Porai·gipa` root — needs a bare-noun-level native confirmation, not an engineering fix. **Verification:** `npm run build` green end-to-end — 215/215 unit tests (up from 203), `test-dictionary.js` 8109/8109 valid, `repository-intelligence.js` PASSED with 0 NEW violations across all checks (Check F's known-mismatch count dropped 292→289, expected/harmless — some previously-allowlisted mismatches were for keys this session's fix correctly stopped shipping), lint 0 errors/warnings, `vite build` clean. `HEAD==origin/main`, fully pushed. PAT supplied live by Project Owner, used inline in the push URL only, never persisted. Full detail: `docs/CLAUDE_B_SESSION_MIGRATION_20260814_E.md`.

**MIGRATION-MODE UPDATE (same session, immediately after the above, no further engineering per explicit Project Owner instruction):** two concurrent Claude A pushes landed while syncing/pushing the work above (`8d165f7`+`5ccb79a`, NV-078 medicine/pill counting + angry raka-count closure; then `a01d729`, angry raka pass 2) — rebased cleanly through both, regenerating `compiled_dict.json`/`compiled_dict_alternates.json`/`category_index.json` fresh via `node prepare-data.js` at each conflict point (both times byte-identical to a plain rebuild, no manual JSON reconciliation needed), rebuilding `dist/` and re-running the **full** gate after each. Final state re-verified against the fully-merged tree: `npm run build` green end-to-end — 215/215 unit tests (unchanged), `test-dictionary.js` **8132/8132** valid (up from 8109 — reflects NV-078's new medicine/pill entries, not this session's own work), `repository-intelligence.js` PASSED 0 NEW violations (Check F still 289 known/0 new), lint clean, vite build clean. Two further docs-only Claude A commits (`56291e4`, `1901a56` — session-close doc + a new `flag_from_claude_a` item, stale `phrase_maps.js` "angry" form) landed immediately after and fast-forward-merged with zero conflict; **not investigated or actioned this session**, entering migration mode instead per explicit instruction to stop. **Final HEAD: `39763a3`** (this session's own last commit; two harmless Claude A docs-only commits sit on top of it at `origin/main` as of migration-mode close, pulled clean, nothing further done). `HEAD == origin/main`, clean tree, zero divergence, fully pushed — reconfirmed at migration-mode close. **Carried forward to next session, neither touched:** (a) `src/data/phrase_maps.js` line 38's stale spaced "angry" form — Claude B's lane, see `.ai/WORKSTATE.yaml`'s `claude_b.flag_from_claude_a`/`pending_next_session`; (b) `student`'s bare-noun root gap (this session's own flagged finding) — Claude A's linguistic call, see `.ai/WORKSTATE.yaml`'s `claude_a.next_action` (added at the top of that block per Rule 10) and `docs/CLAUDE_B_SESSION_MIGRATION_20260814_E.md`'s "Flagged, not fixed" section.

**NEW, 2026-08-13, Claude B — dead-code removal shipped; Check F gap ledger started with real per-item resolutions (17 of 305), not a static handoff; two process near-misses caught before shipping wrong conclusions.** Resumed via `docs/CLAUDE_B_SESSION_MIGRATION_20260812.md`; origin advanced multiple times mid-session (Claude A: counting-QA, hundreds/thousands classifier fix, `ba`/RULE-023, currency-classifier fix, final WORKSTATE checklist) — each pulled clean, full build gate re-verified every time. (1) `e4cb0ce` — deleted `src/gemini.js` (zero imports anywhere) + unused `@google/generative-ai` dep; verified a companion audit claim that `compiled_dict_alternates.json`/`getAlternates()` is dead was **false** (it's live), left untouched. (2) **Near-miss #1 — linguistic:** traced `"wait"` as a shadowing bug (`grammarOverrides` intends `'Damo/Sengbo'`, `corrections.json`'s `'Damo'` wins at runtime). Project Owner confirmed `'Damo/Sengbo'` based on my report; editing `corrections.json` broke 2 regression tests, surfacing **RC-CANDIDATE-015 (2026-07-25)** — `'Damo/Sengbo'` was itself a literal unresolved OCR placeholder, already superseded by native-confirmed values. `corrections.json` was already correct. Reverted immediately, nothing shipped. (3) Per Project Owner directive, did not stop at a static backlog — worked the Check F gap ledger directly, escalating only genuine linguistic decisions. **Near-miss #2 — tooling:** the first gap-analysis script's `normalize()` stripped trailing punctuation, causing silent key collisions (`"eat"` absorbed `"eat!"`'s `compiled_dict.json` value) — the report doc already pushed earlier this session was built from this wrong dataset. Caught by a direct spot-check against `src/compiled_dict.json`. Rebuilt as `scripts/analyze-check-f-gaps.mjs`, replicating `repository-intelligence.js`'s exact matching logic — verified byte-identical counts (305) against a live Check F run; re-run fresh 3 times as `compiled_dict.json` changed underneath from concurrent Claude A pushes. **Resolved with real evidence (17 of 305):** `eat` (not a gap — tooling bug only); `wait` (stale artifact, escalated — `grammarOverrides['wait']` deletion pending Claude A sign-off); `dance` (stale artifact, escalated — `corrections.json`'s `Chroka` is directly native-confirmed per `VERB_INVENTORY.md`/Thangseng 2026-07-14, `master_dictionary.json`'s 4 `"dance"` candidates are all still `UNVERIFIED` and none is even `Chroka` — flagged, their file); `no` (not a bug — `corrections.json` winning already verified correct in `RUNTIME_ENGINEERING_AUDIT_20260803.md`); 12 punctuation-only items (cosmetic, no functional difference). Full ledger + exact per-item continuation process (grep tests/docs → check raw source dicts → classify stale-artifact/not-a-bug/intentional-exception/genuine-linguistic-decision): `docs/CHECK_F_GAP_REPORT_20260813.md`. 288 of 305 items genuinely not yet investigated — multi-session work, process documented so continuation doesn't restart from zero. **Verification:** `HEAD==origin/main` + clean tree at every sync point, full `npm run build` green (203/203, 0 new Check F violations, 305 known unchanged, lint 0 errors, `vite build` clean), no `master_dictionary.json`/`garo_dictionary.json`/`corrections.json` edits shipped. Full detail: `docs/CLAUDE_B_SESSION_MIGRATION_20260813.md`.
...
**NEW, 2026-08-13, Claude A — counting-system compile-pipeline audit: 12 nouns (~230 keys) are shipping already-repudiated SUPERSEDED fabrications live, with zero VERIFIED alternative anywhere in the corpus.** Triggered by relaying native-confirmed `two trees = Bol panggni` (already correct/live, no action needed) and then checking the whole counting system to the same 100%-match standard per Project Owner request. Cross-checked every counting key (1–20) in `compiled_dict.json` against `master_dictionary.json` source status, ~350 keys total. Findings: (1) 146 keys clean — `dog`/`fish`/`person`/`cat`/`apple`/`tree`/`book`, full 1–20, formula sound, consistent with 2026-08-12. (2) `one dog` has a harmless `pickPrimary` tie-break flaw — VERIFIED and SUPERSEDED duplicates happen to share the same Garo string, so it ships correct by coincidence, not correct precedence. (3) **Severe:** `teacher`/`student`/`house`/`car`/`banana`/`rice`/`water`/`food`/`road`/`river`/`mountain`/`village` are shipping the exact bulk-fabricated SUPERSEDED string for nearly every count 1–20, and no VERIFIED replacement exists in the corpus for any of them — this isn't a live/superseded tie-break like `one dog`, there's nothing else to fall back to. This substantially widens the 2026-08-12 handoff (flagged as a 13-key risk; it's actually ~230 keys, already live, not new). Compile-pipeline defect (Claude B's domain), not a linguistic gap — the SUPERSEDED tagging itself is correct. Full report: `docs/CLAUDE_A_COUNTING_QA_20260813.md`. Handoff to Claude B: (a) compile pipeline should ship nothing rather than a SUPERSEDED value when no VERIFIED candidate exists for a key; (b) spot-check whether the `one dog` tie-break class masks a real mismatch elsewhere. Not touched this session (one-task-per-session, docs-only commit, no content edits): supplying the actual VERIFIED replacement values for the 12 affected nouns — partially covered by the already-held native relay (teacher/student/mountain/village/road/banana/car) plus house/rice/water/food, which still need native input.

**NEW, 2026-08-13, Claude A — wrote the held teacher/mountain/village/road/banana/car native relay into the corpus: 120 new VERIFIED counting entries, directly fixing 120 of the ~230 keys flagged in this session's earlier compile-pipeline audit.** Source: the literal count=1 forms recorded in `docs/CLAUDE_A_SESSION_MIGRATION_20260813.md` (`Skigipa` teacher, `A·bri dotsa` mountain, `Song damsa` village, `Rama dilsa` road, `Te·rik ge·sa` banana + `Te·rik akkasa` bunch, `Gari bolsa` car). Roots/classifiers: mountain=`A·bri`+`dot` (no raka), village=`Song`+`dam` (no raka), road=`Rama`+`dil` (no raka), banana=`Te·rik`+`ge·` (raka, same classifier as pen/chair), car=`Gari`+`bol` (no raka — flagged, not resolved, as an orthographic-only collision with the unrelated tree root `Bol`), teacher=`Skigipa`+`sak` (raka, same classifier as person). Mechanically built 1–20 for each from the confirmed count=1 form using the already-VERIFIED NUMBERS suffix table, same formula/precedent as person/tree/apple/book. `one bunch of bananas`=`Te·rik akkasa` added as a single entry (not mechanically expanded — only the count=1 form was relayed). Resolves 5 of the 10 open house/car/road/river/mountain/village/water/food/rice/banana classifier questions, and the teacher portion of the person/student/teacher root conflict (**student remains open, not touched — needs its own session**). Old SUPERSEDED duplicates retained per citation discipline; 120 new intentional dictionary self-consistency conflicts logged to `src/data/known_dictionary_conflicts.json`. Verified: `prepare-data.js` (8151 entries), `test-dictionary.js` (8151/8151 valid), `repository-intelligence.js` (PASSED, 0 new violations post-allowlist), `node --test tests/unit/*.test.js` (203/203). Still open: house/rice/water/food (~19 keys each, still shipping SUPERSEDED fabrications per the audit — need native input); student's root (111-candidate conflict); coin-root VERIFIED-tagging (small, queued).
